...
def _rust_wasm_bindgen_toolchain_impl(ctx):
    ...
    if ctx.attr.wasm_bindgen_test or ctx.attr.webdriver or ctx.attr.browser or ctx.attr.browser_type:
        ...
        if not ctx.attr.browser:
            fail("Not all webdriver attributes provided. Missing `browser` on `{}`".format(ctx.label))
        if not ctx.attr.browser_type:
            fail("Not all webdriver attributes provided. Missing `browser_type` on `{}`".format(ctx.label))

        all_test_files = []
        all_test_files = depset(transitive = [
