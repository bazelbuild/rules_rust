...
async fn test(args: TestArgs) -> anyhow::Result<()> {
    let main = |args: &TestArgs, bazel: &Path, temp_dir: &Path| -> anyhow::Result<()> {
        ...
        let repo_a_hashes = {
            let output = temp_dir.join("repo_a_hashes.json");
            let output_user_root = temp_dir.join("repo_a_output");
        ...
        let repo_b_hashes = {
            let output = temp_dir.join("repo_b_hashes.json");
            let output_user_root = temp_dir.join("repo_b_output");
