...
def _construct_writer_arguments(ctx, test_runner, opt_test_params, action):
    ...
    # Every declared input is staged in runfiles. Strip each generated root,
    # including compiler tools built in an execution configuration. Empty roots
    # are source files: stripping "/" would corrupt every path in the command.
    roots = {file.root.path: True for file in action.inputs.to_list() if file.root.path}
    writer_args.add_all(roots.keys(), format_each = "--strip_substring=%s/")
