...
| `RULES_RUST_CRATE_UNIVERSE_INCOMPATIBLE_GUPPY_RESOLVER` | Resolve crate features and dependencies with [guppy](https://docs.rs/guppy) instead of `cargo tree`. Enabled by default; set to `0` to fall back to the legacy `cargo tree` resolver, which spawns one process per host/target platform pair and is substantially slower. |
