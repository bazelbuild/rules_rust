...
fn validate_settings<T: AsRef<Path>>(
    settings: &mut RazeSettings,
    cargo_workspace_path: &T,
) -> Result<(), RazeError> {
    ...
    validate_crate_settings(settings, cargo_workspace_path.as_ref())?;
...
pub mod tests {
    ...
    "# };
        ...
        let settings =
            load_settings_from_manifest(cargo_toml_path, /*cargo_bin_path=*/ None).unwrap();
        assert_eq!(settings.workspace_path, "//workspace_path/raze".to_owned());
        assert_eq!(settings.genmode, GenMode::Remote);
    }
