...
fn run() -> Result<u8> {
    ...
    let bep_path = env::temp_dir().join(format!("flycheck_bep_{}.json", std::process::id()));
    ...
    //
    // Precedence: CLI flag > env var > workspace-local fallback (rare;
    // would only fire if someone invoked the wrapper directly without
    ...
    let status = Command::new(args.bazel.as_str())
        ...
        .arg("--output_groups=+rustc_output")
        .arg("--keep_going")
    ...
    let bep_path_camino =
        Utf8PathBuf::try_from(bep_path.clone()).context("BEP path was not valid UTF-8")?;
    let stderr_files = match bep::parse_action_stderr_paths(&bep_path_camino, &args.label) {
        Ok(paths) => paths,
        Err(e) => {
